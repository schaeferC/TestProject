...
/**
 * Diese Activity stellt den Hauptmenüpunkt Uni mit seiner Unterpunkte dar.
 * 
 * @author Enis, Dean
 * 
 */
public class AllgemeinUni extends Activity {
	...
	private OnClickListener ocl = new OnClickListener() {
		...
		public void onClick(View v) {
			if (v == aunewsB) {
				ProgressTimerDialog
						.run(AllgemeinUni.this, "Lade Lehrkraftnews");
				startActivityForResult(
						new Intent(BeuthOrgApplication.getAppContext(),
								News.class), News.NEWS_REQUEST_CODE);
			} else if (v == auinfoB) {

				startActivityForResult(
						new Intent(BeuthOrgApplication.getAppContext(),
								UniInfo.class), UniInfo.UNIINFO_REQUEST_CODE);
			} else if (v == aukontaktB) {

				startActivityForResult(
						new Intent(BeuthOrgApplication.getAppContext(),
								UniKontakt.class),
						UniKontakt.UNIKONTAKT_REQUEST_CODE);
			} else if (v == aubackB) {
				...
				startActivityForResult(
						new Intent(BeuthOrgApplication.getAppContext(),
								Survival.class), Survival.SURVIVAL_REQUEST_CODE);
			}
